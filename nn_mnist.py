...
W2 = tf.Variable(np.float32(np.random.rand(100,100)) * 0.1)
b2 = tf.Variable(np.float32(np.random.rand(100)) * 0.1)

h = tf.nn.sigmoid(tf.matmul(x, W1) + b1)
# h = tf.matmul(x, W1) + b1  # Try this!
y = tf.nn.softmax(tf.matmul(h, W2) + b2) #implementación modelo
...
for epoch in xrange(10):  #numero neuronas
